...
fn integration_zero_rhs_all_methods() {
	...
	let t_eval: Vec<f64> = (0..=20).map(|i| x0 + (xend - x0) * (i as f64) / 20.0).collect();
...
fn max_step_and_first_step_controls() {
	...
	for method in all_methods() {
		...
		for w in sol.t.windows(2) {
			let dt = (w[1] - w[0]).abs();
			assert!(dt <= max_step + 1e-12, "dt={} exceeds max_step {} for {:?}", dt, max_step, method);
		}
	}
	...
	for method in all_methods() {
		let opts = Options::builder()
			...
			.build();
		let sol = solve_ivp(&SHO, x0, xend, &y0, opts).expect("solve_ivp failed");
		assert!(sol.t.len() >= 2, "expected at least two time points for {:?}", method);
		let dt0 = (sol.t[1] - sol.t[0]).abs();
		assert!((dt0 - first_step).abs() <= 1e-6, "first step mismatch: got {}, want {} for {:?}", dt0, first_step, method);
	}
}
	...
	for method in [Method::RK23, Method::DOPRI5, Method::DOP853, Method::Radau5] {
		...
		for (yd, ys) in ys_dense.iter().zip(sol.y.iter()) {
			...
			for (a, b) in yd.iter().zip(ys.iter()) {
				assert!((a - b).abs() <= 1e-8, "dense vs stored mismatch: {} vs {}", a, b);
			}
		}
	}
}
...
fn event_detection_all_and_directional() {
	...
	assert!((z1 - pi_over_2).abs() < 5e-3, "first zero ~pi/2, got {}", z1);
	assert!((z2 - three_pi_over_2).abs() < 5e-3, "second zero ~3pi/2, got {}", z2);
